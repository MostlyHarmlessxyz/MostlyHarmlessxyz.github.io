...
```text
https://github.com/MostlyHarmlessxyz/MostlyHarmlessxyz.github.io/actions
```

也可以用 GitHub CLI 查看最近的运行：

```bash
gh run list --repo MostlyHarmlessxyz/MostlyHarmlessxyz.github.io --workflow pages.yml --limit 3
```

查看某一次运行详情：

```bash
gh run view --repo MostlyHarmlessxyz/MostlyHarmlessxyz.github.io
```
...
4. 执行 `npm run lint`。
5. 执行 `npm run build`。
6. 使用 `actions/configure-pages` 配置 Pages。
7. 使用 `actions/upload-pages-artifact` 上传 `dist/`。
8. 使用 `actions/deploy-pages` 发布到 GitHub Pages。

只有 `push` 到 `main` 或手动运行 `workflow_dispatch` 才会部署。Pull request 只做构建检查，不会部署。
...
```text
https://mostlyharmlessxyz.github.io/blog/my-paper-note/
```

如果刚部署完页面还是旧内容，先强制刷新浏览器缓存。GitHub Pages 也可能有几分钟 CDN 缓存。

也可以用命令检查：

```bash
curl -I https://mostlyharmlessxyz.github.io/blog/my-paper-note/
```

看到 `HTTP/2 200` 就说明页面已经在线上可访问。
